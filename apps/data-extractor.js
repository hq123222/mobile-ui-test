...
(function(window) {
    ...
    const HQDataExtractor = {
        ...
        extractDataWithRegex: function(messages, regex, dataType = 'unknown') {
            ...
            console.log(`从${messages.length}条消息中提取到${results.length}个${dataType}数据`);
            return results;
        },
        ...
        extractQQGroups: async function() {
            ...
            allGroupMessages.forEach(item => {
                ...
                if (!groupMap.has(groupId)) {
                    ...
                    if (allChatMessages && allChatMessages.length > 0) {
                        ...
                        for (let msg of allChatMessages) {
                            ...
                            if (groupNameMatch) {
                                ...
                                console.log(`从聊天记录推断群名: ${groupId} -> ${inferredGroupName}`);
                                break;
                            }
                            // 也可以查找其他可能的群名模式
                            ...
                            if (msgContent.includes(groupId)) {
                                ...
                                if (contextMatch) {
                                    ...
                                    if (nameMatch) {
                                        ...
                                        console.log(`从上下文推断群名: ${groupId} -> ${inferredGroupName}`);
                                        break;
                                    }
                                }
                            }
                    ...
                    console.log(`自动推断群组: ${groupId} -> ${inferredGroupName}`);
                }
                ...
                if (groupMap.has(groupId)) {
                    ...
                    console.log(`添加群聊消息: [${messageData.isMyMessage ? '我方' : messageData.sender}] ${messageData.content.substring(0, 20)}... (messageIndex: ${messageData.messageIndex}, matchIndex: ${messageData.matchIndex})`);
            ...
            groupMap.forEach(group => {
                console.log(`排序前群组 ${group.id} 的消息:`, group.messages.map(m => `[${m.isMyMessage ? '我方' : m.sender}] ${m.content.substring(0, 15)}... (idx: ${m.messageIndex}, match: ${m.matchIndex})`));
                ...
                console.log(`排序后群组 ${group.id} 的消息:`, group.messages.map(m => `[${m.isMyMessage ? '我方' : m.sender}] ${m.content.substring(0, 15)}... (idx: ${m.messageIndex}, match: ${m.matchIndex})`));
            });
